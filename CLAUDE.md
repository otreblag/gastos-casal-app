...
| Versão | Data | Resumo |
|---|---|---|
| **1.2.1** | 07-22 | **Versão mobile (só leitura) + log de auditoria + correção de arredondamento.** Página web mobile-first (`mobile/`, GitHub Pages) para consultar os gastos pelo celular com o PC desligado — o app publica um snapshot consolidado na planilha do Google (Apps Script `publicar_snapshot`/`ler_snapshot`); tela de acesso por token, offline com cache, somente leitura. Log de auditoria/debug em `gastos-logs/*.jsonl` (append-only, retenção 90 dias, segredos mascarados, menu "Abrir pasta de logs"). Correção do fantasma de R$ 0,01 no saldo da divisão (soma em centavos inteiros; "dividido" é 50/50 exato sem dívida). |
